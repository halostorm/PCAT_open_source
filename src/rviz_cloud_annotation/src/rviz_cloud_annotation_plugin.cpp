...
QRVizCloudAnnotation::QRVizCloudAnnotation(QWidget *parent) : rviz::Panel(parent), m_nh("~") {
    ...
    */

    {
        ...
        m_nh.param<std::string>(PARAM_NAME_VIEW_GROUND_TOPIC, param_string, PARAM_DEFAULT_VIEW_GROUND_TOPIC);
        m_view_ground_pub = m_nh.advertise<std_msgs::Bool>(param_string, 1);
        ...
        m_auto_plane_pub = m_nh.advertise<std_msgs::Bool>(param_string, 1);

        m_nh.param<std::string>(PARAM_GROUND_TOPIC, param_string, PARAM_DEFAULT_GROUND_TOPIC);
        m_ground_pub = m_nh.advertise<std_msgs::Float32MultiArray>(param_string, 1);
    }
    ...
    // 菜单栏
    {
        ...
        m_auto_plane_action = new QAction("自动生成地面", menu_bar);
    ...
    // 标注类别设置
    {
        ...
        connect(m_annotation_type_group, button_clicked_function_pointer, this,
                &QRVizCloudAnnotation::onSetAnnotationType);
    }
    // 操作模式设置
    {
    ...
    // 标签和颜色切换设置
    {
        ...
        void (QButtonGroup::*button_clicked_function_pointer)(int) = &QButtonGroup::buttonClicked;
        connect(m_page_button_group, button_clicked_function_pointer, this,
                &QRVizCloudAnnotation::onLabelButtonSelected);
    }
    // 当前标记与显示设置
    {
        ...
        connect(view_control_points_button, &QToolButton::toggled, this,
                &QRVizCloudAnnotation::onViewControlPointsToggled);

        QToolButton *view_ground_points_button = new QToolButton(this);
        view_ground_points_button->setText("地面");
        view_ground_points_button->setCheckable(true);
        view_ground_points_button->setChecked(true);
        view_ground_points_button->setSizePolicy(QSizePolicy::Minimum, QSizePolicy::Preferred);
        page_shift_layout->addWidget(view_ground_points_button);
        connect(view_ground_points_button, &QToolButton::toggled, this,
                &QRVizCloudAnnotation::onViewGround);
    }
    // 遮挡系数设置
    {
    ...
    // 数据集设置
    {
        ...
        //回车键信号?  发送者是edit，信号是回车键被按下，接受者是另一个按键，槽(触发函数)是按键被点击
    ...
    // 地面矫正设置
    {
        QBoxLayout *ground_layout = new QBoxLayout(QBoxLayout::TopToBottom);
        main_layout->addLayout(ground_layout);

        // 标题
        QBoxLayout *bbox_type = new QBoxLayout(QBoxLayout::LeftToRight);
        ground_layout->addLayout(bbox_type);
        m_toolbar_group = new QButtonGroup(this);

        QLabel *title1_label = new QLabel("参数说明: "
                                          "矫正后高度, "
                                          "原始高度, "
                                          "最小半径，"
                                          "最大半径，"
        ...
        QBoxLayout *set_ground_layout = new QBoxLayout(QBoxLayout::LeftToRight);
        ground_layout->addLayout(set_ground_layout);

        m_ground_button = new QPushButton(" 矫正地面 ", this);
        m_ground_button->setCheckable(true);
        set_ground_layout->addWidget(m_ground_button);
        connect(m_ground_button, &QPushButton::toggled, this, &QRVizCloudAnnotation::onAutoPlane);

        const char *default_value[7] = {"1.7", "2", "2", "50", "2", "0.2", "15"};
        for (int i = 0; i < 7; i++) {
            m_ground_params_edit[i] = new QLineEdit(default_value[i], this);
            set_ground_layout->addWidget(m_ground_params_edit[i]);
            connect(m_ground_params_edit[i], &QLineEdit::returnPressed, this, &QRVizCloudAnnotation::onGround);
        }
    }
...
void QRVizCloudAnnotation::onViewGround(const bool checked) {
    std_msgs::Bool msg;
    msg.data = checked;
    m_view_ground_pub.publish(msg);
}
...
void QRVizCloudAnnotation::onAutoPlane(const bool checked) {
    std_msgs::Bool msg;
    msg.data = checked;
    m_auto_plane_pub.publish(msg);
}

void QRVizCloudAnnotation::onGround() {
    std_msgs::Float32MultiArray array;

    for (int i = 0; i < 7; i++) {
        array.data.push_back(m_ground_params_edit[i]->text().toFloat());
    }
    m_ground_pub.publish(array);
}
