...
void RVizCloudAnnotation::InitNewCloud(ros::NodeHandle &nh) {
    ...
    m_control_points_weight_sub = m_nh.subscribe(param_string, 1, &RVizCloudAnnotation::onControlPointWeightChange,
                                                 this);
    ...
    m_rect_selection_sub = m_nh.subscribe(param_string, 1,
    ...
    m_view_ground_sub = m_nh.subscribe(param_string, 1, &RVizCloudAnnotation::onViewGround, this);
    ...
    m_ground_sub = m_nh.subscribe(param_string, 1, &RVizCloudAnnotation::onGround, this);

    m_nh.param<double>(PARAM_NAME_AUTOSAVE_TIME, param_double,
                       PARAM_DEFAULT_AUTOSAVE_TIME);
    if (param_double >= 1.0) {
        m_autosave_timer = m_nh.createTimer(ros::Duration(param_double),
    ...
    m_view_cloud = m_view_labels = m_view_control_points = m_view_ground = true;
...
void RVizCloudAnnotation::LoadCloud(const std::string &filename,
    ...
    static auto remove_nan = Factory<CloudFilter<PXYZRGBN>,
            const Params &>::BuildT(ns("RemoveNaNFilter"), Params{});
    remove_nan->filter(m_cloud, m_cloud);
    ...
    ground_ind.clear();
    if (is_seg_ground) {
        auto ground_seg = Factory<GroundEstimator<PXYZRGBN>,
                const Params &>::BuildT(
                ns("RingShapedElevationConjunctionMap"),
                Params{
                        {"sensor_height", sensor_height},
                        {"th_g",          th_g},
                        {"max_slope",     max_slope},
                        {"num_scan",      (int) 30},
                        {"delta_ring",    delta_range},
                        {"min_ring",      min_range},
                        {"max_ring",      max_range}
                });


        std::vector<int> non_ground_ind;
        ground_seg->estimate(m_cloud, ground_ind, non_ground_ind);

        if (ground_ind.size() < 20) {
            ROS_INFO("rviz_cloud_annotation: too less ground points");
            return;
        }
        PCXYZRGBNPtr ground(new PCXYZRGBN(*m_cloud, ground_ind));
        pcl::ModelCoefficients::Ptr coeff(new pcl::ModelCoefficients);
        pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
        pcl::SACSegmentation<PXYZRGBN> seg;
        ...
        Eigen::Vector3f normal = Eigen::Vector3f(coeff->values[0], coeff->values[1], coeff->values[2]).normalized();
        normal = normal.z() < 0 ? -normal : normal;

        float dis_from_origin_to_ground = coeff->values[3], height = 2.0f;
        Eigen::Isometry3f T_og = Eigen::Isometry3f::Identity();
        auto t_og = (-dis_from_origin_to_ground + height) * normal;  // 底面距离原点 -dis_from_origin_to_ground
        auto r_og_axis = Eigen::Vector3f::UnitZ().cross(normal).normalized();   // 求取地面在世界中的位姿
        auto r_og_angle = acosf(Eigen::Vector3f::UnitZ().dot(normal));
        T_og.translate(t_og);
        T_og.rotate(Eigen::AngleAxisf(r_og_angle, r_og_axis));

        pcl::transformPointCloud(cloud, cloud, T_og.inverse());
        ROS_INFO("rviz_cloud_annotation: cloud size: %ld", cloud.size());
    }
}
...
RVizCloudAnnotation::CloudToMarker(const PointXYZRGBNormalCloud &cloud,
    ...
    if (m_view_cloud && m_view_ground) {
        for (auto it: ground_ind) {
            cloud_marker.colors[it].r = 1;
            cloud_marker.colors[it].g = 1;
            cloud_marker.colors[it].b = 1;
        }
    }
...
// ou1ong 添加地面生成
void RVizCloudAnnotation::ground_generate() {
    InitNewCloud(*nh);
}
